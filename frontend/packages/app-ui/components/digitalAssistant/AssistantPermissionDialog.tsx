...
import {
	...
	type DigitalAssistantPermissionMember,
	type DigitalAssistantPermissionRole,
	type DigitalAssistantPermissionSettings,
	type DigitalAssistantVisibility,
	digitalAssistantApi,
	type UserInfo,
	userApi,
} from "@leros/store";
...
type AssistantPermissionDialogProps = {
	open: boolean;
	onOpenChange: (open: boolean) => void;
	assistant: Pick<DigitalAssistantItem, "publicId" | "name" | "permissionRole">;
	onSaved?: (settings: DigitalAssistantPermissionSettings) => void;
};

const ROLE_LABELS: Record<DigitalAssistantPermissionRole, string> = {
	owner: "所有者",
	admin: "管理员",
	member: "使用者",
};

function userLabel(user: DigitalAssistantPermissionMember["user"]) {
	return user.name || user.email || user.public_id;
}

function userSecondaryLabel(user: DigitalAssistantPermissionMember["user"]) {
	return user.email || "组织成员";
}

function toMember(user: UserInfo): DigitalAssistantPermissionMember {
	return {
		user: {
...
export function AssistantPermissionDialog({
	open,
	onOpenChange,
	assistant,
	onSaved,
}: AssistantPermissionDialogProps) {
	const [settings, setSettings] = useState<DigitalAssistantPermissionSettings | null>(null);
	const [visibility, setVisibility] = useState<DigitalAssistantVisibility>("private");
	const [members, setMembers] = useState<DigitalAssistantPermissionMember[]>([]);
	...
	const canEditMembers =
		assistant.permissionRole === "owner" || assistant.permissionRole === "admin";
	const canChangeVisibility = assistant.permissionRole === "owner";
	...
	useEffect(() => {
		...
		setLoading(true);
		digitalAssistantApi
			.getPermissions(assistant.publicId)
			.then((response) => {
				if (cancelled) return;
				const next = response.data.data;
				if (!next) throw new Error("权限数据为空");
				setSettings(next);
	...
	const addMember = (user: UserInfo) => {
		...
		setMembers((current) => [...current, toMember(user)]);
		setSearch("");
		setCandidates([]);
	};

	const updateMemberRole = (publicID: string, role: DigitalAssistantPermissionRole) => {
		setMembers((current) =>
			current.map((member) =>
	...
	const handleSave = async () => {
		...
		setSaving(true);
		try {
			const response = await digitalAssistantApi.updatePermissions({
				public_id: assistant.publicId,
				visibility: canChangeVisibility ? visibility : settings.visibility,
				members: members.map((member) => ({
					user: { public_id: member.user.public_id },
					role: member.role,
				})),
			});
			const next = response.data.data;
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="flex max-h-[min(88dvh,760px)] w-[min(620px,94vw)] max-w-none flex-col gap-0 overflow-hidden p-0 sm:rounded-2xl">
				<DialogHeader className="shrink-0 border-b border-[var(--leros-control-border)] px-6 py-5 pr-14">
					<DialogTitle className="text-xl text-[var(--leros-text-strong)]">
						共享 AI 队友
					</DialogTitle>
					...
					<div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
						...
						<section>
								...
								<div className="relative mb-3">
									...
									{search.trim() && (
										<div className="absolute inset-x-0 top-11 z-20 overflow-hidden rounded-lg border border-[var(--leros-control-border)] bg-white shadow-lg">
											...
											) : candidates.filter((user) => !memberIDs.has(user.public_id)).length > 0 ? (
												candidates
														...
														>
															...
															<span className="min-w-0 flex-1">
																<span className="block truncate text-sm text-[var(--leros-text-strong)]">
																	{user.name || user.email || user.public_id}
																</span>
											...
											<Select
												...
												onValueChange={(value) =>
													updateMemberRole(
														...
														(value ?? "member") as DigitalAssistantPermissionRole,
													)
												}
											>
												<SelectTrigger
							...
							<div className="mt-4 rounded-lg bg-[var(--leros-primary-soft)] px-3 py-2.5 text-xs leading-5 text-[var(--leros-text-muted)]">
								...
								<div className="mt-1">
									<span className="font-semibold text-[var(--leros-text-strong)]">使用者</span>
									　只能查看和使用队友。
								</div>
							</div>
						</section>
