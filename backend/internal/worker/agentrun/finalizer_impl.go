...
func (f *finalizer) FinalizeRequired(
	...
	result := &agentrundomain.RunResult{
		...
		Usage:       agentUsageToDomain(runtimeResult.Usage),
		ToolCalls:   agentToolCallsToDomain(runtimeResult.ToolCalls),
		Artifacts:   artifacts,
...
func collectArtifacts(
...
) ([]messaging.RunEventBody, []messaging.ArtifactPayload, error) {
	plan, ok := workspacePlan(workspace)
	if !ok {
		return nil, nil, nil
	}
	...
	events := make([]messaging.RunEventBody, 0, len(filtered))
	artifacts := make([]messaging.ArtifactPayload, 0, len(filtered))
	for _, record := range filtered {
		payload := artifactPayloadFromRecord(record)
		events = append(events, messaging.RunEventBody{
		...
		artifacts = append(artifacts, messaging.ArtifactPayload{
			...
			PreviousRelativePath: payload.PreviousRelativePath,
...
func artifactPayloadFromRecord(record agentworkspace.ArtifactRecord) messaging.ArtifactPayload {
	return messaging.ArtifactPayload{
		...
		PreviousRelativePath: strings.TrimSpace(record.PreviousPath),
